...
static BOOL CALLBACK install_after_runtime(PINIT_ONCE once, PVOID parameter, PVOID *context) {
    ...
    if (!ml_asset_hooks_install_render_ready(game)) {
        ML_LOG_WARN(L"armoredcore6", L"AFTER_RENDER_READY trigger HOOK_FAILED; CPU affinity deferred capability disabled");
    }
    if (!ml_lifecycle_advance(ML_LIFECYCLE_PHASE_AFTER_RUNTIME_INIT)) {
        ML_LOG_WARN(L"armoredcore6", L"AFTER_RUNTIME_INIT lifecycle advance failed");
    }
    ...
    ml_log_write(assets_applied ? ML_LOG_LEVEL_INFO : ML_LOG_LEVEL_WARN,
                 L"armoredcore6", assets_requested
                     ? (assets_applied
...
bool armoredcore6_install(void) {
    const ml_game_descriptor_t *game = ml_game_context_get();
    if (game == NULL || game->id != ML_GAME_ARMORED_CORE_6 ||
    ...
    return image_base != NULL && image_size != 0 &&
           ml_runtime_ready_hook_install(L"armoredcore6", install_after_runtime);
}

void armoredcore6_uninstall(void) {
    if (!ml_asset_hooks_uninstall()) {
        ML_LOG_WARN(L"armoredcore6", L"one or more asset hooks could not be removed");
    }
    if (ml_runtime_ready_hook_uninstall(L"armoredcore6")) {
        image_base = NULL;
        image_size = 0;
    }
}
